...
| a **live-model** dogfood on the native backend   | NOT TESTED     | artifact:docs/alpha8-status.md | Partially closed: a packaged install ran a real session on the native backend (§6), but with the fake model. Placing a provider credential on that host is the user's decision, not the agent's. [open:A3]                                                                                    |
