...
| a release tag whose own gate is green exists            | NOT TESTED | artifact:docs/alpha9-status.md | Deferred to `v0.1.0-alpha.8.1` under alpha.9 §19 CLOSURE A, and closed there rather than here. **Closed by alpha.9** [closed:C4] |
...
| **the first-choice provider could not be used**          | NOT TESTED | artifact:docs/alpha8-cross-model.md                                                                  | OpenAI directly returned `insufficient_quota`; the account has no credit. Recorded as ENVIRONMENT_ERROR rather than worked around. [open:A4] |
...
| Requirement                                      | Status         | Evidence                       | Notes                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------------ | -------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| strict public-address egress on a clean resolver | NOT APPLICABLE | artifact:docs/alpha8-status.md | Restated at the time, not closed. Both available hosts NAT public names into 198.18.0.0/15; the positive control cannot be produced here. **Downgraded from `NOT TESTED` by alpha.10 §15**, along with the alpha.6, alpha.7 and alpha.9 rows for the same claim; reason in ADR-0017 [open:A7]                                               |
| a **live-model** dogfood on the native backend   | NOT TESTED     | artifact:docs/alpha8-status.md | Partially closed: a packaged install ran a real session on the native backend (§6), but with the fake model. Placing a provider credential on that host is the user's decision, not the agent's. **Closed by alpha.11** — `docs/alpha11-native-live-dogfood.md`. Kept at its alpha.8 status because alpha.8 used the fake model [closed:C5] |
