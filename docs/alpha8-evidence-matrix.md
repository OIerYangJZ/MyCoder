...
| the release gate has been observed green           | NOT TESTED |                                          | It has never run: this repository has no configured remote CI for the alpha.8 branch, and the workflow is asserted by reading rather than by a run. The install path it encodes _was_ exercised by hand on the evidence host (§1, §6). |
