...
| a credential refused for a workspace that swallowed the config directory blames the workspace | PASS   | test:a workspace that contains the config directory blames the workspace, not the credential | `mycoder` in `$HOME`: the credential was correctly placed and the advice would have moved it somewhere worse                 |
| the remedy names `--cwd` rather than moving the credential                                    | PASS   | test:the remedy names --cwd rather than moving the credential                                | And the ordinary case keeps `inside-workspace` and its own remedy                                                            |
| a workspace containing the config directory is refused whatever the credential source         | PASS   | test:a workspace containing the config directory is refused, with an env credential          | ADR-0028. The `api_key_env` route started a session with workspace-dev over the whole home directory                         |
| NEGATIVE CONTROL: an ordinary project workspace still starts                                  | PASS   | test:NEGATIVE CONTROL: an ordinary project workspace is accepted                             | A project under `$HOME` is nearly every project; the test is containment of the config directory                             |
