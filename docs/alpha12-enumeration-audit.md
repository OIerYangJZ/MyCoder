...
**117 enumerations** in `src/` and `scripts/`. **22 guarded, 15 declared unguarded,
80 closed by design.** Two of the mirrors had already drifted, and one of those two
...
| `INTERESTING`                  | `src/cli/render.ts`                     | CLOSED  | the tool-call arguments worth putting on a line, in the order they are looked for |
