...
## 0. What this milestone reached, and the one thing it did not
...
denied" — is false as written and cannot be made true while MCP exists. The
original is kept visible in the plan at §25.1, the reason at §25.2, the
replacement at §25.3, and §23's Trust Stop is replaced at §25.4. The replacement
clause is stricter in one respect: it requires the limitation to be stated _in
the product_, not merely to be true. See `docs/alpha9-status.md`.
...
| `/status` reports the MCP dimension to the **user**        | PASS   | test:attaching a server changes what /status is allowed to say                  | ControlHost carries the session descriptor, not the backend one                |
| only the local backend can host a stdio server             | PASS   | test:only the local backend implements session(), asserted from source          | A backend growing session() fails this, loudly                                 |
| a protected-path call is not claimed to be prevented       | PASS   | test:the kernel does not claim to prevent it, and says so                       | Asserting it was blocked would be the overclaim ADR-0023 section 6 prevents    |
| a bad MCP config exits CONFIG, per ADR-0021                | PASS   | test:a bad MCP configuration is CONFIG, not a generic failure                   | Not the catch-all                                                              |
| defect 5: credential_ref on stdio is refused, not ignored  | PASS   | test:is refused, not silently ignored                                           | Refusing beats guessing; both beat silence                                     |
...
| **CLOSURE C — strict egress on a clean resolver**        | NOT APPLICABLE | artifact:docs/alpha9-status.md                                              | Third milestone at the time. **Downgraded to NOT APPLICABLE by alpha.10 §15** — both hosts NAT public names into 198.18/15, re-verified 2026-08-16, and a fourth restatement was declined. Reason recorded in ADR-0017 [open:A7] |
