...
**Maintained from 2026-08-16 (alpha.10).** One index for every claim that is not
...
saying `NOT TESTED`, because nothing reconciles eight matrices against each other:
...
`scripts/evidence.ts` cannot catch either. It checks that a claim **points at
something that exists**; it does not check that a claim is **still true**, that a
closed one was back-annotated, or that one claim has one status. Both are legal
states of a legal document.

So this index is the reconciliation, by hand until it is gated. Rule: **an item
leaves this file only when the row that carried it is annotated in its own
matrix.** Deleting from here without that is how the drift starts again.
...
| Claim                                     | Closed by                                      |
| ----------------------------------------- | ---------------------------------------------- |
| OS-level isolation of a child             | alpha.5 — "Subagent + Container green"         |
| host-scoped network allowlist enforcement | alpha.6 — the scoped egress proxy              |
| Tool usability: success rate, step counts | alpha.7 §B — the friction table and the runner |
| a release tag whose own gate is green     | alpha.9 CLOSURE A — `v0.1.0-alpha.8.1`         |

---

## What would make this file unnecessary

A gate. `scripts/evidence.ts` already parses every matrix; it could require that
every non-`PASS` row appears in this index, and that one claim never carries two
statuses. That turns reconciliation from a habit into a check, which is the same
move the project made for enforcement descriptors and packed contents.

It is a candidate for alpha.11 and is written up there rather than done here,
because a gate added in the same commit as the list it gates has never been run
against a repository that violates it.
