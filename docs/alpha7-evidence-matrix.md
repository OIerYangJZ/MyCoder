...
| A live-model dogfood on the native backend (§57)          | NOT TESTED     |          | needs the provider credential on the Linux host, which is the user's decision to make rather than the agent's [open:A3]                                                                                                                                    |
