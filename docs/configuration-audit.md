...
| `[security] approval_mode`       | the rule that an action the policy engine flagged is put to the user: `accept-edits` answers workspace writes, and `auto` also answers deletions and commands | credentials, network, git history and MCP tools ask in every mode; `deny` and `hard_deny` never become approvals at all, so privilege escalation, protected paths and host-environment reads stay refused whatever the mode is. No mode can grant what a policy layer denied — a mode only answers questions the layers already decided to raise        | **user only** | loadConfig drops a project-declared `security.approval_mode` with a warning; mergeSecurity keeps the stricter of two modes, so a layer may only ask _more_ |
