...
| Windows Docker Desktop status recorded         | NOT TESTED |                                                                                    | No trustworthy runner. §39 permits compatibility-only status; no claim is made for Windows containers [open:A6] |
...
| ------------------------------------------ | -------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| container/kernel zero-day resistance       | NOT APPLICABLE |          | Out of scope; §36 documents the threat model [scope]                                                                                                                                                                                                                                         |
| VM-level isolation                         | NOT APPLICABLE |          | Not attempted; §5 lists Firecracker/gVisor/Kata as non-goals [scope]                                                                                                                                                                                                                         |
| native-Linux-equivalent isolation on macOS | NOT TESTED     |          | Cannot be established on a VM-backed daemon; the kernel's own platform note says so [open:B1]                                                                                                                                                                                                |
| host-scoped network allowlist enforcement  | NOT TESTED     |          | Would need an egress proxy in the network namespace; reported `best-effort` everywhere. **Closed by alpha.6**, which built exactly that proxy — exact-host matching and port scope, each with a reverse control. Kept at its alpha.5 status because alpha.5 did not establish it [closed:C2] |
| production multi-tenant sandbox            | NOT APPLICABLE |          | One user, one workspace, one session [scope]                                                                                                                                                                                                                                                 |
| Docker daemon compromise resistance        | NOT APPLICABLE |          | The daemon is trusted infrastructure in this design [scope]                                                                                                                                                                                                                                  |
| billing-grade delegated cost               | NOT TESTED     |          | Still parked from alpha.4 §57; monetary values remain labelled `estimated` [open:A5]                                                                                                                                                                                                         |
| parallel Agents / automatic delegation     | NOT APPLICABLE |          | §71 forbids adding them here [scope]                                                                                                                                                                                                                                                         |
| clean direct-OpenAI behavioural validation | NOT TESTED     |          | Parked (§56); the relay's hidden prompt makes attribution unsound [open:A4]                                                                                                                                                                                                                  |
