...
| second provider validated live               | NOT APPLICABLE |                                                                                     | §5 NON-GOAL. Provider breadth is an alpha.5 candidate        |
| Anthropic validated live                     | NOT APPLICABLE |                                                                                     | §5 NON-GOAL; no credential configured                        |
...
| ---------------------------------------- | -------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| parallel subagents                       | NOT APPLICABLE |          | §5 NON-GOAL. Delegation is sequential; the parent's step blocks                                                                                                                                                       |
| agent teams / swarm                      | NOT APPLICABLE |          | §5 NON-GOAL                                                                                                                                                                                                           |
| unbounded recursive delegation           | NOT APPLICABLE |          | §12: depth 1 by default, and depth > 1 is not validated                                                                                                                                                               |
| background independent agents            | NOT APPLICABLE |          | §19: a child cannot outlive its parent's turn                                                                                                                                                                         |
| OS-level isolation of a child            | NOT TESTED     |          | A child is a policy scope in the same process. `describeSandbox` says so; container isolation is the alpha.5 candidate                                                                                                |
| automatic semantic skill routing         | NOT APPLICABLE |          | §22 NON-GOAL. Activation is explicit: control, definition or a bounded model request                                                                                                                                  |
| remote `agentd`                          | NOT APPLICABLE |          | §5 NON-GOAL                                                                                                                                                                                                           |
| delegated cost is a billed amount        | NOT TESTED     |          | Cost is _estimated_ from configured list prices; the pricing page does not list the `deepseek-chat` alias, so the tier cannot be verified. See status §4                                                              |
| grandchild budget attribution at depth>1 | NOT APPLICABLE |          | Closed by making it unreachable: `SYSTEM_CEILING.maxDelegationDepth = 1` clamps every layer, so no configuration reaches the state whose accounting was approximate. test:a config asking for depth 3 is clamped to 1 |
